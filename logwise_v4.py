#!/usr/bin/env python3
"""
LogWise - Linux Syslog RCA Web Tool with Dual Timestamp Format Support,
...
def parse_bracketed_syslog_datetime(line):
    """
    Parses bracketed log lines like:
    [Tue Sep  9 12:24:35 2025] ...
...
def parse_traditional_syslog_datetime(line, year):
    """
    Parses traditional syslog lines like:
    Sep 18 15:42:01 ...
    Uses provided year to return datetime or None.
    """
...
def detect_timestamp_format(log_content):
    """
    Checks the log content for presence of bracketed full timestamps.
    Returns 'bracketed' if found, else 'traditional'.
    """
...
# === Detect year and extract timestamps ===

def detect_year_and_times(log_content, timestamp_format):
    """
    Detect year and extract list of datetimes depending on format.
    For traditional syslog, infer year.
    For bracketed, year is explicit.
    """
...
def filter_syslog_by_time(log_content, start, duration_minutes, year, timestamp_format):
    """
    Returns log lines within [start, start+duration).
    Uses correct parser based on format.
    """
...
def main():
    st.title("LogWise - AI-Driven Log Analysis & Rapid Troubleshooting")
    st.write("Accelerate incident resolution with AI-powered log analysis.")

    # Ollama config hidden in secrets.toml (example below)
    # [ollama]

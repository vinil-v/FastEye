#!/usr/bin/env python3
"""
LogWise - Linux Syslog RCA Web Tool with Auto Date/Time Detection, Dropdown Selection, and Duration
"""
...
def parse_syslog_datetime(line, year):
    """
    Parses a Linux syslog line for its datetime. 
    Expects format: 'MMM DD HH:MM:SS ...', uses provided year.
...
def detect_year_and_times(log_content):
    """
    Detect the year by scanning for any four-digit year in logs, 
    else guessing from current year.
    Extracts all datetimes (with time) from the logs and returns 
    unique sorted datetime list.
    """
    # Try to find year from lines that contain a four-digit year
...
def filter_syslog_by_time(log_content, start, duration_minutes, year):
    """
    Filter syslog lines between start datetime and start+duration.
    """
...
def main():
    ...
    st.write("Upload a Linux syslog file to auto-detect log date/time, select event, and analyze.")
    ...
    if uploaded_file:
        ...
        st.markdown(f"**Detected event times (sorted):**")

        # Format times as strings for dropdown selection (HH:MM:SS)
        time_options = [dt.strftime('%Y-%m-%d %H:%M:%S') for dt in detected_times]
        selected_time_str = st.selectbox("Select exact event time from detected timestamps:", time_options)
